                               ...
                               std::vector<int64_t>& sorted_indices)
{
    ...
    if (sort_by_row)
    {
        ...
        std::sort(sorted_indices.begin(), sorted_indices.end(),
                  [const &matrix, index](int64_t i1, int64_t i2) { return matrix.circ_at(index, i1) < matrix.circ_at(index, i2); });
    }
    else
    {
        // Sorting based on a specific column
        std::sort(sorted_indices.begin(), sorted_indices.end(),
                  [const &matrix, index](int64_t i1, int64_t i2) { return matrix.circ_at(i1, index) < matrix.circ_at(i2, index); });
    }
}
//-------------------------------------------------------------------
