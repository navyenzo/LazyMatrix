...
#ifndef INCLUDE_PADDING_HPP_
#define INCLUDE_PADDING_HPP_
...
struct PaddedMatrix : public BaseMatrix< PaddedMatrix<MatrixType> >
{
    ...
    PaddedMatrix<MatrixType>(const MatrixType& expression, uintptr_t padded_rows, uintptr_t padded_columns, const value_type& constant_value_for_padding = static_cast<value_type>(0))
...
struct is_type_a_matrix< PaddedMatrix<MatrixType> > : std::true_type
{
};
//-------------------------------------------------------------------
...
pad_matrix(const MatrixType& m,
           uintptr_t padded_rows,
           uintptr_t padded_columns,
           const DataType& constant_value_for_padding)
{
    return PaddedMatrix<MatrixType>(m, padded_rows, padded_columns, constant_value_for_padding);
}
//-------------------------------------------------------------------
